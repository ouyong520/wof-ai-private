...
#${ROOT_ID} .wof-anchor{position:absolute;left:50%;top:25%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;gap:8px}
...
#${ROOT_ID} .wof-alert{position:relative;min-width:210px;text-align:center;border-radius:15px;padding:11px 16px 13px;background:rgba(0,0,0,.84);border:3px solid rgba(255,255,255,.94);box-shadow:0 5px 22px rgba(0,0,0,.60);overflow:hidden;white-space:nowrap;color:#fff}
...
#${ROOT_ID} .wof-status{position:absolute;left:8px;bottom:8px;padding:6px 9px;border-radius:9px;background:rgba(0,0,0,.52);font-size:10px;opacity:.78;color:#fff}
#${ROOT_ID}.wof-placing{pointer-events:auto;cursor:crosshair;background:rgba(0,0,0,.04)}
...
`;
  ...
  root.innerHTML='<div class="wof-place-tip">点击画面位置固定HUD</div><div class="wof-anchor"><div class="wof-alerts"></div><div class="wof-point"><span class="bigdot">●</span><span class="label">P1</span></div></div><div class="wof-status">P1 · 固定HUD</div>';
  document.body.appendChild(root);
  const anchor=root.querySelector('.wof-anchor'),alerts=root.querySelector('.wof-alerts'),point=root.querySelector('.wof-point'),label=root.querySelector('.label'),status=root.querySelector('.wof-status');

  let visible=true,detail=false,focus='P1',lastMsg=null,lastRx=0,destroyed=false,placing=false;
  ...
  function position(){anchor.style.left=(POS_X*innerWidth)+'px';anchor.style.top=(POS_Y*innerHeight)+'px';}
  ...
  function render(msg){
    ...
    status.textContent=focus+' · 固定HUD '+Math.round(POS_X*100)+'% / '+Math.round(POS_Y*100)+'%';
  ...
  function click(e){
    if(!placing)return;POS_X=clamp(e.clientX/innerWidth,.08,.92);POS_Y=clamp(e.clientY/innerHeight,.10,.75);cancelPlace();position();
    console.log('🎯 HUD固定位置已设为',Math.round(POS_X*100)+'%',Math.round(POS_Y*100)+'%');e.preventDefault();e.stopPropagation();
  }
  ...
  window.WOFHUD={
    version:'hud-overlay-v10-fixed-front-back',show(){visible=true;return true;},hide(){visible=false;return true;},toggle(){visible=!visible;return visible;},
    ...
    place:armPlace,pos(x,y){if(x!=null)POS_X=clamp(+x,.08,.92);if(y!=null)POS_Y=clamp(+y,.10,.75);position();return{x:POS_X,y:POS_Y};},
    status(){return{visible,detail,focus,pos:{x:POS_X,y:POS_Y},lastRx,lastMsg,hold:HOLD[focus]};},
  ...
  console.log('✅ WOF HUD overlay v10 fixed-position + front/back threat started');
  console.log('⚠ 普通风险：显示“注意 + 前方怪/后方怪 + 左/右侧”；动作风险：显示“↑上躲/↓下躲/AB + 威胁方向”。');
  console.log('📌 暂停错误人物跟随：HUD固定不漂。F7可点画面任意位置重新摆放；F6切P1/P2/P3。');
})();
