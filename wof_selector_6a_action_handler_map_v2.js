...
// WOF live program ROM window is 1 MiB in the existing probes. 0x81774 is safely inside it.
// Do not widen into following HEAP data: that creates false code/table hits.
const MAX=Math.min(0x100000,M.length-base);
...
// Exact selected-player -> player+0x29 -> action dispatch bridge checks.
const bridge={
...
// These relative-word dispatch tables place the first code target immediately after the table,
// so first relative offset == table byte length. This prevents reading following opcodes as entries.
...
for(let type=0;type<TYPES;type++){
  ...
  typeRows.push({type,at:h(at),tableBase:h(ptr),ptr,valid:okAddr(ptr)});
}
...
function refNeighborhood(p){
  const out=[];
  for(let q=Math.max(0,p-16)&~1;q<=Math.min(MAX-4,p+16);q+=2){
    const v=r32(q)>>>0;
    out.push({at:h(q),long:h(v,8),validCode:okAddr(v),mark:q===p?'FOCUS_REF':''});
  }
  return out;
}
const literalRefs=[];
for(let p=0;p+3<MAX;p+=2){
  const v=r32(p)>>>0;
  if(FOCUS.has(v))literalRefs.push({at:h(p),target:h(v),...classifyRef(p),neighborhood:refNeighborhood(p)});
}

// Direct dispatcher exits near the action targets. This does not claim full CFG reachability;
// it only surfaces exact local exits if they exist.
...
const verdict={
  version:'wof-selector-6a-action-handler-map-v2.1',readOnly:true,ramWrites:0,
  romScanLimit:h(MAX),
  priorBugsFixed:['v1 stopped at 0x30000 before type35 table 0x81774','v2 draft widened past the known 1MiB program ROM; v2.1 clamps back to 0x100000'],
...
const out={
  version:'wof-selector-6a-action-handler-map-v2.1',verdict,bridge,
