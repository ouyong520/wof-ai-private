...
let oldLoc=null;try{oldLoc=OLD?.diagnose?.()?.locator||null;}catch(_){}
try{OLD?.stop?.();}catch(_){}
const MOD=_0x515056;if(!MOD?.HEAPU8)throw new Error('HEAPU8 unavailable');
const ROM_LIMIT=0x100000,NT=47;
...
let stopped=false,LOC=null,base=-1,SWAP=false,MAX=0,DISPATCH=null;
...
function setLoc(b,sw,label){base=b;SWAP=sw;MAX=Math.min(ROM_LIMIT,heap().length-base);LOC={base,swap16:SWAP,label,sp:0x00FF62EE,pc:0x0000754A};DISPATCH=findDispatch();if(!DISPATCH)throw new Error('ROM found but confirmed type dispatch sequence not found');console.log('✅ ROM ready','base=0x'+base.toString(16).toUpperCase(),'layout='+(SWAP?'swap16':'direct'),'dispatch='+h(DISPATCH));return LOC;}
...
function findDispatch(){if(!LOC)return null;let best=null;for(let o=0x2400;o<=0x2700;o+=2){let n=0;for(let i=0;i<EXPECT.length;i++){if(r32(o+i*4)===EXPECT[i])n++;else break;}if(n>=3&&(!best||n>best.n))best={o,n};}if(best){console.log('🎯 dispatch calibrated',h(best.o),'matched '+best.n+'/'+EXPECT.length);return best.o;}
  const o=0x25DC,vals=EXPECT.map((_,i)=>r32(o+i*4)),d=vals[0]-EXPECT[0],same=vals.every((v,i)=>v-EXPECT[i]===d);console.warn('⚠️ confirmed dispatch sequence not found near 0x25DC',{at25DC:vals.map(h),uniformDelta:same?h(d):null});return null;}
async function locate(opts={}){if(LOC)return LOC;stopped=false;const M=heap();
  if(oldLoc&&Number.isInteger(oldLoc.base)&&vecOK(M,oldLoc.base,!!oldLoc.swap16)){console.log('♻️ reuse previous ROM location');return setLoc(oldLoc.base,!!oldLoc.swap16,'reused-v3');}
...
async function result(){if(!LOC)throw new Error('先执行 await WOFFOCUSROM.locate()');const {TYPES}=model();console.log('=== calibrated type table ===');console.table(TYPES.map(x=>({type:x.type,entry:h(x.entry),valid:x.entry<MAX,sharedWith:TYPES.filter(y=>y.entry===x.entry&&y.type!==x.type).map(y=>y.type).join(',')})));console.log('🔎 分片扫描玩家引用…');const refs=await playerRefs();console.log('=== direct 32-bit P1/P2/P3 refs ===');console.table(refs.longRefs);console.log('=== low-16 P1/P2/P3 refs ===');console.table(refs.wordRefs.slice(0,120));const helpers=commonHelpers(refs);console.log('=== common helper candidates ===');console.table(helpers.slice(0,50));const out={version:'rom-focus-probe-v4-dispatch-calibrated',vectors:{sp:h(r32(0)),pc:h(r32(4)),romBaseHeap:'0x'+base.toString(16).toUpperCase(),dispatchTable:h(DISPATCH),swap16:SWAP},types:TYPES,longRefs:refs.longRefs,wordRefs:refs.wordRefs,helpers:helpers.slice(0,100)};self.__WOF_ROM_FOCUS_LAST=out;return out;}
function routine(type,span=0x700){const x=model().TYPES[type];const calls=callsIn(x.entry,span).map(c=>({at:h(c.at),target:h(c.target),kind:c.kind}));console.table(calls);return{type,entry:h(x.entry),calls,hex:hex(x.entry,96)};}
...
self.WOFFOCUSROM={version:'rom-focus-probe-v4-dispatch-calibrated',get located(){return!!LOC;},locate,result,routine,dump,diagnose(){return{heapBytes:heap().length,located:!!LOC,locator:LOC,dispatch:DISPATCH==null?null:h(DISPATCH)};},stop(){stopped=true;}};
console.log('✅ WOF ROM focus probe v4 loaded · 会复用上一版ROM位置');
