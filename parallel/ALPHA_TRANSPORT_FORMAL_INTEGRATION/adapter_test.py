...
class FakeAdapter(mod.FormalRealAdapter):
    ...
    def _install_worker(self, worker_id, binding):
        ...
        return {"running": True, "identitySignature": mod.IDENTITY_SIGNATURE,
                "identity": {"ok": True, "sha256": mod.GOLDEN_SHA, "identitySignature": mod.IDENTITY_SIGNATURE,
                             "readOnly": True, "ramWrites": 0, "inputInjection": False},
                "readOnly": True, "ramWrites": 0, "inputInjection": False,
                "workerReplacement": False, "queueDepth": 0}
...
def exact_current_authority_only():
    ...
    a.worker_status={"running":True,"identitySignature":mod.IDENTITY_SIGNATURE,
                     "identity":{"ok":True,"sha256":mod.GOLDEN_SHA,"identitySignature":mod.IDENTITY_SIGNATURE},
                     "runtimeEpoch":b.runtime_epoch,"session":b.session,"pairGeneration":b.pair_generation,"pairNonce":b.pair_nonce,
                     "queueDepth":0,"readOnly":True,"ramWrites":0,"inputInjection":False,"workerReplacement":False}
...
def detector_local_identity_required_for_current_authority():
    a=FakeAdapter(); b=old_binding(); a.current=b
    a.page_status={"bound":True,"transportVersion":mod.TRANSPORT,"session":b.session,"pairGeneration":b.pair_generation,"pairNonce":b.pair_nonce}
    a.worker_status={"running":True,"identitySignature":mod.IDENTITY_SIGNATURE,
                     "identity":{"ok":True,"sha256":"0"*64,"identitySignature":mod.IDENTITY_SIGNATURE},
                     "runtimeEpoch":b.runtime_epoch,"session":b.session,"pairGeneration":b.pair_generation,"pairNonce":b.pair_nonce,
                     "queueDepth":0,"readOnly":True,"ramWrites":0,"inputInjection":False,"workerReplacement":False}
    assert not a._current_still_authoritative(choice(page=b.page_id,worker=b.worker_id))
check("current authority requires fresh detector-local exact SHA",detector_local_identity_required_for_current_authority)
...
def normal_bind_orders_revoke_before_bind_install():
    ...
    assert a.last_binding and a.last_binding["launcherIdentitySha"] == PASS_ID["sha256"]
check("normal rebind revokes before fresh generation install and carries measured Discovery SHA",normal_bind_orders_revoke_before_bind_install)
...
print({"schema":"wof-alpha-formal-adapter-test-v2","status":"FAIL" if failed else "PASS","testCount":len(results),"passCount":len(results)-len(failed),"failCount":len(failed),"results":results})
