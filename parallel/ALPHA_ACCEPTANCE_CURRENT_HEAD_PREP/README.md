...
- `repository_preflight.py` — authoritative successor-aware repository gate policy.
- `acceptance_entrypoint.py` — official entrypoint; injects the current repository policy into the existing bounded acceptance runtime without changing Browser/WOF logic.
...
- `RUN_CURRENT_HEAD_ACCEPTANCE.cmd` — Windows wrapper; now calls `acceptance_entrypoint.py`.
- `test_repository_preflight.py` — deterministic repository-policy adversarial regression.
...
Before Browser access, `repository_preflight.py` requires all of the following to be current and green. A claim being merely `COMPLETE` is insufficient; the expected PASS/decision/stop condition and release-consumed blob pins are checked.
...
8. Only after all evidence gates are green, the existing local deterministic Safe Transport/Formal/PYLAUNCH offline regressions are rerun before Browser admission.
...
## Head-label live acceptance boundary
...
Repository synthetic QA, source inspection, and an `UNPROVEN` projection profile are not accepted as live projection proof. `--preflight-only` therefore reports repository PASS separately and explicitly says bounded live target-label visual acceptance remains pending.
...
3. The tool reruns the current successor-aware repository preflight first. Any non-green gate exits before Browser access; do not bypass it.
