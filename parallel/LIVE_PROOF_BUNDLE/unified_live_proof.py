...
STOP_CONDITION = "UNIFIED LIVE PROOF READY â€” ONE OWNER WOF RUN REMAINS"
RECORDER_ADMISSION_MARKERS = (
    "World 921031 å·²ç¡®è®¤ / Discovery V2 / åªè¯»æ¨¡å¼",
    "World 921031 å·²ç¡®è®¤ / åªè¯»æ¨¡å¼",
)
    ...
    for port in range(start, end + 1):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            try:
                ...
                continue
            return port
    raise RuntimeError("æ²¡æœ‰å¯ç”¨çš„æœ¬æœº WOF proof CDP ç«¯å£ï¼ˆ9423-9499ï¼‰ã€‚")
...
def run_live(project_root: Path) -> int:
    ...
    try:
        print("æ­£åœ¨å¯åŠ¨ 1 ä¸ªä¸“ç”¨ WOF æµè§ˆå™¨æˆ¿é—´â€¦â€¦")
        manager.start(1)
        ...
        print()
        print("ä¸“ç”¨æµè§ˆå™¨å·²æ‰“å¼€ã€‚ç°åœ¨åªéœ€è¦åœ¨è¿™ä¸ªæµè§ˆå™¨ä¸­æ­£å¸¸è¿›å…¥ä¸€ä¸ª WOF æˆ¿é—´ã€‚")
        print("ç»Ÿä¸€éªŒè¯ä¼šè‡ªåŠ¨æ£€æŸ¥ï¼š")
        print("  Browser / WOF é¡µé¢ / Worker / WASM / World 921031")
        print("  Browser Fleet Discovery V2")
        print("  WOF-052L Recorder Discovery V2 å‡†å…¥")
        ...
        while True:
            while True:
                try:
                    ...
                    break
                if line and ("å·²ç¡®è®¤" in line or "å¤±è´¥" in line or "æ‹’ç»" in line or "ERROR" in line):
                    print(f"[{prefix}] {line}")
            ...
            if launcher_proc.poll() is not None and not pylaunch.get("automatedPass"):
                msg = f"PYLAUNCH å·²æå‰é€€å‡ºï¼ˆcode={launcher_proc.returncode}ï¼‰ã€‚"
                if msg not in blockers:
                    blockers.append(msg)
            if recorder_proc.poll() is not None and not recorder_evidence.admitted:
                msg = f"WOF-052L Recorder å·²æå‰é€€å‡ºï¼ˆcode={recorder_proc.returncode}ï¼‰ã€‚"
                if msg not in blockers:
                    blockers.append(msg)
            if recorder_evidence.fatal and recorder_evidence.fatal_line:
                msg = f"Recorder å‡†å…¥å¤±è´¥ï¼š{recorder_evidence.fatal_line}"
                if msg not in blockers:
                    blockers.append(msg)
            ...
            if now - last_display >= 2.0:
                print(
                    "\r"
                    f"æµè§ˆå™¨ï¼š{'å·²è¿æ¥' if fleet['browser'] else 'ç­‰å¾…'}ï½œ"
                    f"WOF é¡µé¢ï¼š{'å·²æ‰¾åˆ°' if fleet['page'] else 'ç­‰å¾…'}ï½œ"
                    f"Fleet Workerï¼š{'å·²æ‰¾åˆ°' if fleet['workerIndicator'] else 'ç­‰å¾…'}ï½œ"
                    f"PYLAUNCH Worldï¼š{'å·²ç¡®è®¤' if pylaunch['world921031'] else 'ç­‰å¾…'}ï½œ"
                    f"Recorderï¼š{'å·²å‡†å…¥' if recorder_evidence.admitted else 'ç­‰å¾…'}      ",
                    end="",
                    flush=True,
                )
                last_display = now
            ...
            if payload["live"]["automatedChecksReady"]:
                print()
                print()
                print("è‡ªåŠ¨åªè¯»éªŒè¯å…¨éƒ¨é€šè¿‡ã€‚")
                print("è¯·åªç¡®è®¤æœ€åä¸€ä¸ªçœŸäººäº‹å®ï¼šå½“å‰ WOF æˆ¿é—´æ˜¯å¦ä»èƒ½æ­£å¸¸è¿è¡Œï¼Ÿ")
                answer = input("æ­£å¸¸è¯·è¾“å…¥ Yï¼›å¦‚æœæ¸¸æˆå¼‚å¸¸è¯·è¾“å…¥ Nï¼š").strip().lower()
    ...
    if final["overallResult"] == "PASS":
        print("  PASS â€” å·²å…·å¤‡ 10 æˆ¿é—´é•¿é‡‡é›†æ¡ä»¶")
        print("  æœ¬å·¥å…·æ²¡æœ‰è‡ªåŠ¨å¼€å§‹ä¸€å°æ—¶é•¿é‡‡é›†ã€‚")
    else:
        print("  çœŸäººçŸ­éªŒè¯æœªå®Œæˆ")
        print("  ä¸€å¤„å¤±è´¥ä¸ä¼šè¦†ç›–å…¶ä»–å·²å–å¾—è¯æ®ã€‚")
    print("============================================================")
    print(f"æ€»ç»“æœ JSONï¼š{status_path}")
