...
- P42, when present, is archived as diagnostic correlation evidence only (`authorityEligible=false`); if absent the receipt records `P42_CORRELATION_PROBE_NOT_PRESENT`.
...
- exact candidate/attestation/manifest/pointer/provenance path, SHA-256, Git blob, package version and runtime pins;
...
- P42 raw bundle and artifact hash/path when a `wof-gstyphoon-renderer-correlation-probe-v1` surface exists, otherwise `P42_CORRELATION_PROBE_NOT_PRESENT`;
...
- every CDP session and the P36 observer are deterministically torn down.
