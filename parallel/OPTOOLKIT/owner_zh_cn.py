...
EXACT = {
    ...
    "4 Start Browser Fleet": "4 启动多房间浏览器", "5 Run Regression": "5 运行回归测试", "6 Run Live Proof": "6 运行真人 Windows 验证",
...
def translate_text(text: str) -> str:
    ...
    prefixes = [
        ...
        ("Project updated: ", "项目已更新："), ("Started: ", "已启动："), ("Could not start: ", "启动失败。技术详情："),
        ("Recorder output: ", "采集器结果目录："), ("Diagnostics saved: ", "诊断信息已保存："),
        ("Package ready: ", "结果包已生成："), ("Opened: ", "已打开："),
        ("WOF Toolkit could not validate project root:", "WOF 工具箱无法确认项目根目录："),
    ]
    for old,new in prefixes:
        if text.startswith(old): return new + text[len(old):]
    if text.startswith("This action timed out."): return "本次操作超时。没有进行游戏内存写入，也没有发送游戏输入。"
    if text.startswith("Toolkit could not complete this action:"):
        detail=text.split(":",1)[1].strip() if ":" in text else text; return f"工具箱没有完成这项操作。\n技术详情：{detail}"
    if text.startswith("Please choose a number from 0 to 9."): return "请输入 0 到 9 之间的菜单编号。"
    if text.startswith("alpha_product:"): return text.replace("alpha_product:","Alpha 产品回归：",1)
    if text.startswith("rc5_independent_bootstrap:"): return text.replace("rc5_independent_bootstrap:","RC5 独立 bootstrap 回归：",1)
    if text.startswith("wof052l_self_test:"): return text.replace("wof052l_self_test:","WOF-052L 自检：",1)
    if text.startswith("pylaunch_unittest:"): return text.replace("pylaunch_unittest:","PYLAUNCH 单元测试：",1)
    if text.startswith("browser_fleet_unittest:"): return text.replace("browser_fleet_unittest:","Browser Fleet 单元测试：",1)
    if text.startswith("toolkit_unittest:"): return text.replace("toolkit_unittest:","Operator Toolkit 单元测试：",1)
    if "BLOCKED (Node.js not found)" in text: return text.replace("BLOCKED (Node.js not found)","受阻（未找到 Node.js）")
    if text.endswith(": MISSING"): return text[:-len(": MISSING")]+"：缺失"
    return text

def translated_print(*args, **kwargs): _ORIGINAL_PRINT(*(translate_text(x) if isinstance(x,str) else x for x in args), **kwargs)
...
class ChineseToolkit(toolkit.Toolkit):
    ...
    def component(self,key):
        if key=='recorder':
            print("\n[启动多房间采集器]"); entry=self.root/"parallel/WOF052L_RECORDER/owner_zh_cn.py"
            if not entry.is_file(): print("没有找到多房间采集器中文入口。请先选择 1“更新项目”。"); return
            out=self.results/"recorder"; out.mkdir(parents=True,exist_ok=True); ok,detail=self.spawn(entry,["--output-dir",str(out)])
            if ok: print("已启动："+detail); print("采集器结果目录："+str(out))
            else: print("启动失败。"); print("游戏本身没有受到影响。"); print("技术详情："+detail)
            return
        return super().component(key)

    def proof(self):
        print("\n[运行真人 Windows 验证]")
        supervisor=self.root/"parallel/OPTOOLKIT/live_session.py"; launcher=self.root/"parallel/PYLAUNCH/launcher.py"
        if not supervisor.is_file() or not launcher.is_file(): print("没有找到完整的真人验证组件。请先选择 1“更新项目”。"); return
        session_dir=self.results/f"live_session_{toolkit.stamp()}"; session_dir.mkdir(parents=True,exist_ok=True)
        ...
        print("正常流程：进入游戏后请正常玩；工具会自动准备 Camera 和头顶定位。")
        print("只有在画面明确提示时，最多点击一次 P1 头顶上方希望提示出现的位置；之后继续正常玩即可。")
        print("无需选择数学模型，也无需记住左右/纵深/跳跃/resize 检查清单；确实缺少某一种动作时，画面只会提示当前唯一下一步。")
...
def main()->int:
    ...
    if not (root/"parallel/PYLAUNCH").is_dir() or not (root/"product/alpha").is_dir(): print("无法确认 WOF 项目目录。"); print(f"技术详情：{root}"); return 2
    builtins.print=translated_print; builtins.input=translated_input
    try: return ChineseToolkit(root).loop()
