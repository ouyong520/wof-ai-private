...
def _safe_upload(root: Path, zip_path: Path, session_dir: Path) -> dict[str, Any]:
    ...
    A generic `gh` command is intentionally not enough authority to choose a remote
    destination or evidence retention policy. If a future package provides the
    approved uploader at this fixed path, it may reuse the machine's pre-existing
    non-interactive auth. Until then evidence stays local.
    """
    uploader = root / "parallel" / "OWNER_ONECLICK" / "upload_live_evidence.py"
    if not uploader.is_file():
        return {"attempted": False, "status": "LOCAL_ONLY_NO_REPOSITORY_DEFINED_SECURE_UPLOADER"}
    try:
        cp = subprocess.run(
            [sys.executable, str(uploader), "--zip", str(zip_path), "--session-dir", str(session_dir)],
            cwd=str(uploader.parent), text=True, encoding="utf-8", errors="replace",
    ...
    return {
        "attempted": True,
        "status": "UPLOADED" if cp.returncode == 0 else "UPLOAD_FAILED_LOCAL_ZIP_RETAINED",
        "returnCode": cp.returncode,
        "detail": (cp.stdout or cp.stderr or "")[-1000:],
    }
...
def run_session(root: Path, session_dir: Path) -> int:
    ...
    results_root = session_dir.parent
    packages = results_root / "packages"
    zip_path = packages / f"WOF_LIVE_ACCEPTANCE_{session_dir.name}.zip"
    proof_json = session_dir / "WINDOWS_PROOF_STATUS.json"
    stdout_path = session_dir / "launcher.stdout.txt"; stderr_path = session_dir / "launcher.stderr.txt"
    launcher = root / "parallel" / "PYLAUNCH" / "launcher.py"
    ...
    try:
        with stdout_path.open("w", encoding="utf-8") as out, stderr_path.open("w", encoding="utf-8") as err:
            cp = subprocess.run(
                [sys.executable, str(launcher), "--proof-json", str(proof_json), "--activate-alpha", "--package-root", str(root)],
                cwd=str(launcher.parent), env=os.environ.copy(), stdout=out, stderr=err, check=False,
            )
            rc = cp.returncode
    except Exception as exc:
    ...
    try:
        _zip_session(session_dir, zip_path)
        upload = _safe_upload(root, zip_path, session_dir)
    except Exception as exc:
        upload = {"attempted": False, "status": "PACKAGING_FAILED_RAW_EVIDENCE_RETAINED", "detail": str(exc)}
    summary["upload"] = upload
    summary["zipReady"] = zip_path.is_file()
    _write_json(session_dir / "SESSION_SUMMARY.json", summary)
    # Refresh the zip once so its included summary carries upload/fallback state.
    if zip_path.is_file():
...
def main() -> int:
    parser = argparse.ArgumentParser(description="WOF live acceptance auto evidence supervisor")
    parser.add_argument("--root", required=True)
    parser.add_argument("--session-dir", required=True)
    args = parser.parse_args()
    return run_session(Path(args.root), Path(args.session_dir))
