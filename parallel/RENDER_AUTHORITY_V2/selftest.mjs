...
assert.ok(result.candidateTimeline.every(f=>f.runtimeEpoch==='0123456789abcdef'));
assert.ok(result.candidateTimeline.every(f=>f.rendererEpoch==='fedcba9876543210'));
assert.ok(result.candidateTimeline.every(f=>f.authorityKey==='authority-v2'));
assert.ok(result.candidateTimeline.every(f=>f.regions.every(r=>r.authority==='UNVERIFIED_CANDIDATE_ONLY')));
assert.equal(Object.hasOwn(result,'rendererSourceProof'),false);
