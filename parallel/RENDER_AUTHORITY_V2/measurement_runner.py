...
def run(root: Path, output_root: Path, host: str, port: int, browser: str, browser_path: str | None) -> int:
    root = root.resolve(); output_root = output_root.resolve()
    ...
    print("正常进入 WOF 并正常玩 20-30 秒；无需校准、无需点击头顶、无需做指定动作。")
    print("在 renderer authority 尚未证明前，旧投影标签会被关闭，宁可暂时不显示也不显示错位置。")
    ...
    if endpoint is None:
        ...
        try:
            ...
            event("BLOCKED", reason=str(exc)); print("BLOCKED：无法启动只读调试浏览器。", exc); return 4
    if endpoint is None:
        event("BLOCKED", reason=rejection or "browser endpoint unavailable"); print("BLOCKED：浏览器调试端口不可用。", rejection or ""); return 5

    client = CdpClient(endpoint.websocket_url, timeout=5.0); client.connect()
    guard = RuntimeAuthorityGuard(); identity_cache: dict[str, dict] = {}
    capture = RenderAuthorityCapture(lambda rel: (root / rel).read_text(encoding="utf-8"))
    accepted = None; authority_key = None; runtime_epoch = None; page_surface = None
    try:
        while True:
            if accepted is None:
                ...
                ok = bool(choice.page and choice.worker and choice.worker_probe and choice.worker_probe.get("moduleOk") is True and choice.identity and choice.identity.get("ok") is True)
                if not ok:
                    print("等待 exact World 921031 Page/Worker/WASM；正常进入游戏即可。", end="\r", flush=True)
                    time.sleep(1.0); continue
                fp = guard.accept(client, choice)
                authority_key = fp.key(); runtime_epoch = secrets.token_hex(16); accepted = choice
                page_surface = _page_cleanup_and_surface(client, str(choice.page.get("targetId")))
                event("EXACT_AUTHORITY_ACCEPTED", authorityKey=authority_key, worldSha256=choice.identity.get("sha256"), runtimeEpoch=runtime_epoch, pageSurface=page_surface)
                capture.ensure_started(client, choice, authority_key, runtime_epoch)
            ...
            if not healthy:
                ...
                capture.stop_runtime(client); guard.clear(); identity_cache.clear(); accepted=None; authority_key=None; runtime_epoch=None
                print("运行时已更换，旧采集已自动撤销；正在重新发现新 Worker。")
            ...
            if isinstance(remote, dict):
                print(f"自动采集：{remote.get('sampleCount',0)} samples / {remote.get('candidateCount',0)} structural candidates", end="\r", flush=True)
            if polled.get("state") == "ERROR":
                event("CAPTURE_ERROR", error=polled.get("error")); raise RuntimeError(str(polled.get("error")))
            if polled.get("state") == "MEASUREMENT_COMPLETE":
                result = polled.get("result")
                if not isinstance(result, dict):
                    raise RuntimeError("terminal capture result missing")
                result["pageSurface"] = page_surface
                result["sessionSafety"] = SAFETY
                _write(session_dir / "RENDER_AUTHORITY_CAPTURE_RESULT.json", result)
                event("MEASUREMENT_COMPLETE", sampleCount=result.get("sampleCount"), candidateCount=len(result.get("candidateRegions") or []))
                summary = {"schema": SCHEMA, "startedAt": started, "endedAt": datetime.now().astimezone().isoformat(timespec="seconds"), "verdict": "BOUNDED_CAPTURE_READY_FOR_RENDER_AUTHORITY_ANALYSIS", "ownerAction": "NORMAL_PLAY_ONLY", "worldSha256": result.get("worldSha256"), "runtimeEpoch": result.get("runtimeEpoch"), "authorityKey": result.get("authorityKey"), "sampleCount": result.get("sampleCount"), "candidateCount": len(result.get("candidateRegions") or []), "legacyProjectionUsed": False, "manualCalibrationUsed": False, "productionOverlaySuppressedUntilAuthorityVerified": True, "safety": SAFETY, "zipPath": str(zip_path)}
                _write(session_dir / "SESSION_SUMMARY.json", summary); _zip_dir(session_dir, zip_path)
                (session_dir / "FINAL_ZIP.txt").write_text(str(zip_path)+"\n", encoding="utf-8")
                print("\n自动采集完成。结果包：" + str(zip_path))
                return 0
            time.sleep(0.75)
    except KeyboardInterrupt:
        event("OWNER_STOPPED"); return 130
    except Exception as exc:
        event("CAPTURE_FAILED", error=str(exc)); print("\n采集失败，已保留现有只读证据：", exc); return 6
    finally:
...
def main() -> int:
    ...
    p.add_argument("--root", required=True); p.add_argument("--output-root", required=True)
    p.add_argument("--host", default="127.0.0.1"); p.add_argument("--port", type=int, default=9223)
    p.add_argument("--browser", choices=["auto","chrome","edge"], default="auto"); p.add_argument("--browser-path")
    a=p.parse_args(); return run(Path(a.root),Path(a.output_root),a.host,a.port,a.browser,a.browser_path)
