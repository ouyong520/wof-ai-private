...
const DURATION_MS=30000,TICK_MS=250,MAX_EVENTS=96,MAX_REGIONS=48,REGION_BYTES=96,SCAN_BYTES=0x20000;
...
function structuralScore(M,p,swap){const rd=swap?u16le:u16be;let good=0,moving=0,nonzero=0;for(let i=0;i<8;i++){const q=p+i*8;if(q+7>=M.length)break;const x=rd(M,q),y=rd(M,q+2),tile=rd(M,q+4),attr=rd(M,q+6);if((x&0x1ff)<0x1a0&&(y&0x1ff)<0x120)good++;if((x|y)!==0)moving++;if(tile!==0&&tile!==0xffff&&attr!==0xffff)nonzero++;}return good*3+moving+nonzero;}
function topStructuralRegions(M,start,end,limit=6){const top=[],s=Math.max(0,start&~1),e=Math.min(M.length-REGION_BYTES,end);for(let p=s;p<e;p+=8){for(const swap of [false,true]){const score=structuralScore(M,p,swap);if(score<31)continue;const row={heapOffset:p,byteOrder:swap?'LE16':'BE16',structuralScore:score,rawHex:hex(M,p,REGION_BYTES),authority:'UNVERIFIED_CANDIDATE_ONLY'};top.push(row);top.sort((a,b)=>b.structuralScore-a.structuralScore||a.heapOffset-b.heapOffset);if(top.length>limit)top.length=limit;}}return top;}
...
function start(binding){
  stop('restart');
  if(!binding||binding.worldSha256!==WORLD_SHA||typeof binding.runtimeEpoch!=='string'||binding.runtimeEpoch.length<16||typeof binding.authorityKey!=='string'||!binding.authorityKey)throw new Error('exact World/runtime authority binding invalid');
  ...
  state={schema:SCHEMA,state:'MEASURING',terminal:false,captureComplete:false,captureCompletedAt:null,continuousActorFeed,startedAt:started,endedAt:null,runtimeEpoch:binding.runtimeEpoch,authorityKey:binding.authorityKey,worldSha256:binding.worldSha256,moduleKey:found.key,heapBytes,ramBase:R||null,moduleSurface:surface(found.mod),sampleCount:0,scanBytes:0,actorSamples:[],currentActors:{players:[],enemies:[],ramBase:R||null,sampleAt:started},candidateRegions:[],p1Lifecycle:{active:false,generation:0,type:null,x:null,y:null,z:null},ownerActionZh:'正常玩；Camera 会先自动识别 P1 身份并尝试零点击定位场景 P1 头部；只有无法安全唯一确认时才允许最多一次实际头部点击。',measurementRequired:'exact runtime renderer/object authority plus bounded zero-click-first P1 head visual authority',...SAFETY};
  const addEvent=(kind,payload={})=>{events.push({at:Date.now(),kind,...payload});if(events.length>MAX_EVENTS)events.shift();};addEvent('MEASUREMENT_STARTED',{durationMs:DURATION_MS,continuousActorFeed});
  let tick=0;
  const sample=()=>{
    if(!state||state.terminal)return;
    if(state.runtimeEpoch!==binding.runtimeEpoch||state.authorityKey!==binding.authorityKey){stop('authority-generation-mismatch');return;}
    ...
    if(!state.captureComplete){
      ...
      const ranked=[...regions.values()].sort((x,y)=>y.structuralScore-x.structuralScore||x.heapOffset-y.heapOffset).slice(0,MAX_REGIONS);regions.clear();for(const r of ranked)regions.set(r.byteOrder+':'+r.heapOffset,r);state.candidateRegions=ranked;state.scanBytes+=SCAN_BYTES*2;tick++;
      if(now-started>=DURATION_MS){
        state.captureComplete=true;state.captureCompletedAt=now;state.resultVerdict='BOUNDED_CAPTURE_READY_FOR_RENDER_AUTHORITY_ANALYSIS';addEvent('MEASUREMENT_COMPLETE',{samples:state.sampleCount,candidates:ranked.length});
...
G.WOFRENDERAUTHV2={schema:SCHEMA,start,status,result,stop,_test:{u16be,u16le,structuralScore,topStructuralRegions}};
})();