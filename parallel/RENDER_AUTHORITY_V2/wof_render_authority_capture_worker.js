...
function start(binding){
  ...
  const sample=()=>{
    ...
    if(!state.captureComplete){
      ...
      if(tick%2===0&&ranked.length){state.candidateTimeline.push({at:now,sequence:state.sampleCount,p1Lifecycle:{...state.p1Lifecycle},regions:ranked.slice(0,TIMELINE_REGIONS).map(r=>snapshotRegion(M,r))});if(state.candidateTimeline.length>MAX_TIMELINE)state.candidateTimeline.shift();}
      tick++;
