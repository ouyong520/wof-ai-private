...
class DiscoveryV2Tests(unittest.TestCase):
    ...
    def test_stale_and_worker_replacement(self):
        p={"targetId":"p","type":"page","url":"https://host/wof"}; w1={"targetId":"old","type":"worker","url":"https://cdn/a.js"}; w2={"targetId":"new","type":"worker","url":"https://cdn/b.js"}; cache={}
        first=discover(FakeClient([p,w1],page={"p":{"gameSurface":True}},stale={"old"}),identity_cache=cache); self.assertEqual("p",first.page["targetId"])
        second=discover(FakeClient([p,w2],page={"p":{"gameSurface":True}},light={"new":GOOD_LIGHT},identity={"new":GOOD_ID}),identity_cache=cache); self.assertEqual("new",second.worker["targetId"])
