...
class CanonicalDrawEvidenceCollectorTests(unittest.TestCase):
    ...
    def test_exact_identity_is_accepted_and_atomically_written(self):
        ...
        self.assertEqual(result["schema"], "wof-alpha-canonical-draw-evidence-v1")
        self.assertEqual(result["version"], 1)
        self.assertEqual(result["identity"]["pageTargetId"], PAGE_ID)
        self.assertEqual(result["identity"]["runtimeEpoch"], AUTHORITY["runtimeEpoch"])
        self.assertEqual(result["pageTarget"]["id"], PAGE_ID)
