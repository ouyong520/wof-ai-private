...
class WindowsProofTests(unittest.TestCase):
    ...
    def test_pass_requires_all_six_checks_without_alpha_mode(self) -> None:
    ...
    def test_alpha_mode_requires_package_runtime_running(self) -> None:
        snapshot = self._passing_snapshot()
        snapshot.update(alpha_requested=True, alpha_running=False)
        result = compact_proof_snapshot(snapshot)
        self.assertEqual("WAITING", result["automatedResult"])
        self.assertEqual("--", result["checks"]["Alpha V1 package runtime"])
        snapshot["alpha_running"] = True
        result = compact_proof_snapshot(snapshot)
        self.assertEqual("PASS", result["automatedResult"])
        self.assertEqual("OK", result["checks"]["Alpha V1 package runtime"])

    def test_nonzero_ram_writes_fails_closed(self) -> None:
        snapshot = self._passing_snapshot(); snapshot["ram_writes"] = 1
        result = compact_proof_snapshot(snapshot)
        self.assertEqual("WAITING", result["automatedResult"])
        self.assertEqual("--", result["checks"]["READ ONLY / RAM writes: 0"])

    def test_missing_identity_fails_closed(self) -> None:
        snapshot = self._passing_snapshot(); snapshot["world_921031"] = False
        result = compact_proof_snapshot(snapshot)
        self.assertEqual("WAITING", result["automatedResult"])
    ...
    def test_writer_emits_single_compact_json(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual("wof-python-launcher-windows-proof-v2", payload["schema"])
