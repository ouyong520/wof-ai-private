...
from .canonical_runtime_coordinator import (
    CanonicalRuntimeCoordinator,
    CanonicalRuntimeCoordinatorError,
)
...
class AlphaRuntimeManager:
    """Install one package-selected Alpha lifecycle under exact World authority.

    The canonical product path is P12 identity/generation -> P10 deterministic
    render-object anchor -> P9/P8 -> maintained P11 HUD. W3 remains an explicit
    producer seam and is never synthesized or polled here.

    Legacy projection recovery remains available only for the pre-existing
    non-canonical compatibility surfaces. Its readiness never gates canonical
    semantic publication or canonical W3 anchor ingestion.
    """
    ...
    def __init__(self, root: Path) -> None:
        ...
        self._canonical = CanonicalRuntimeCoordinator(self._verified_text)
    ...
    def _verified_bytes(self, rel: str) -> bytes:
        ...
        if actual != wanted:
            raise AlphaRuntimeError(
                f"package-selected runtime 完整性失败：{rel} expected={wanted} actual={actual}"
            )
        return data
    ...
    def _page_install(
        ...
        try:
            ...
            raise AlphaRuntimeError(
                f"package-selected Alpha page source injection failed: {bootstrap_rel}: {exc}"
            ) from exc

        base = self._evaluate(
            client,
            page_id,
            "(()=>{const s=window.__WOF_ALPHA_BOOTSTRAP_RC5,t=window.__WOF_ALPHA_TRANSPORT_V1;return {version:s?.version,session:s?.session,channel:s?.channel,transportVersion:t?.version,listenerReady:s?.listenerReady===true};})()",
        )
        if (
            not isinstance(base, dict)
            or base.get("listenerReady") is not True
            or base.get("transportVersion") != TRANSPORT
        ):
        ...
        pair = self._evaluate(
            client,
            page_id,
            f"window.__WOF_ALPHA_TRANSPORT_V1.bind({json.dumps(pair_nonce)})",
        )
        if (
            not isinstance(pair, dict)
            or pair.get("bound") is not True
            or not isinstance(pair.get("pairGeneration"), int)
        ):
        ...
        for rel, source in page_sources[1:]:
            try:
                self._evaluate(
                    client,
                    page_id,
                    f"(0,eval)({json.dumps(source)}); true",
                    timeout=15.0,
                )
            except Exception as exc:
                raise AlphaRuntimeError(
                    f"package-selected Alpha page source injection failed: {rel}: {exc}"
                ) from exc
        ...
        if missing:
            raise AlphaRuntimeError(
                "package-selected Alpha canonical HUD capability 缺失: "
    ...
    def _profiles_for_worker(self) -> dict[str, dict[str, Any]]:
        ...
        if live is not None:
            return live
        return {
            name: json.loads(self._verified_text(rel))
    ...
    def _worker_install(
        ...
        if (
            ...
            or not isinstance(locator.get("heapBase"), int)
            or not isinstance(locator.get("swap16"), bool)
        ):
        ...
        core, labels, player_warning, adapter = (
            self._verified_text(p) for p in WORKER_SOURCES
        )
        self._evaluate(
            client,
            worker_id,
            "try{self.__WOF_ALPHA_REAL_TRANSPORT?.stop?.('field-rebind')}catch(_){}; true",
        )
        ...
        self._evaluate(
            client,
            worker_id,
            f"self.__WOF_ALPHA_FIELD_PROFILES={json.dumps(profiles, ensure_ascii=False)}; true",
        )
        self._evaluate(
            client,
            worker_id,
            f"(0,eval)({json.dumps(adapter)}); true",
            timeout=15.0,
        )
        binding = {
            "release": RELEASE,
            ...
            "launcherLocator": {
                "heapBase": locator["heapBase"],
                "swap16": locator["swap16"],
            },
        }
        status = self._evaluate(
            client,
            worker_id,
            f"self.WOFAlphaFieldAdapter.install(self,{json.dumps(binding)})",
            await_promise=True,
            timeout=30.0,
        )
        if (
            ...
            or status.get("readOnly") is not True
            or status.get("ramWrites") != 0
            or status.get("inputInjection") is not False
        ):
            raise AlphaRuntimeError(
                "package-selected Alpha field adapter 未进入安全 running 状态"
            )
        local_identity = status.get("identity")
        if (
            not isinstance(local_identity, dict)
            or local_identity.get("sha256") != WORLD_SHA256
            or local_identity.get("ok") is not True
        ):
            raise AlphaRuntimeError(
                "package-selected Alpha field adapter detector-local identity 未通过"
            )
        return status

    def _sync_canonical_status(self) -> dict[str, Any]:
        status = self._canonical.status()
        if self._status.get("running") is True:
            self._status["canonicalOverlay"] = status
        return status
    ...
    def ensure_running(
        ...
        page_id = str(choice.page.get("targetId") or "") if choice.page else ""
        worker_id = str(choice.worker.get("targetId") or "") if choice.worker else ""
        if (
            self._authority_key == authority_key
            and self._status.get("running") is True
            and page_id == self._page_target_id
            and worker_id == self._worker_target_id
        ):
            self._sync_canonical_status()
            return dict(self._status)

        self.revoke(client)
        if (
            not choice.page
            or not choice.worker
            or not choice.identity
            or choice.identity.get("ok") is not True
        ):
        ...
        try:
            ...
            worker_status = self._worker_install(
                client,
                worker_id,
                pair,
                choice.identity,
            ...
            page_status = self._evaluate(
                client,
                page_id,
                "(()=>({attachState:window.__WOF_ALPHA_BOOTSTRAP_RC5?.attachState||null,hudLoaded:!!window.WOFALPHAHUD,hudStatus:window.WOFALPHAHUD?.status?.()||null}))()",
            )
            ...
            canonical_status = self._canonical.activate(
                client,
                page_id,
                authority_key=authority_key,
                runtime_epoch=runtime_epoch,
                world_sha256=str(choice.identity.get("sha256") or ""),
                capability_present=pair["canonicalOverlayCapable"] is True,
            )

            projection_status = self._projection.status()
            if self._projection.profiles() is None:
                projection_status = self._projection.ensure_started(
                    client,
                    choice,
                    authority_key,
                )
        ...
        self._status = {
            ...
            "canonicalOverlay": canonical_status,
    ...
    def ingest_canonical_w3_frame(
        ...
        *,
        ...
        """Explicit stable seam for the W3 producer; never polls or guesses W3."""
        if self._status.get("running") is not True:
            raise AlphaRuntimeError(
                "canonical W3 frame rejected because AlphaRuntime is not running"
            )
        try:
            status = self._canonical.ingest_frame(
                frame,
                sample_at=sample_at,
                requested_actors=requested_actors,
            )
        except (CanonicalRuntimeCoordinatorError, CdpError, OSError, ValueError) as exc:
            self._canonical.revoke("CANONICAL_W3_INGEST_ERROR")
            self._sync_canonical_status()
            raise AlphaRuntimeError(f"canonical W3 ingest failed: {exc}") from exc
        self._status["canonicalOverlay"] = status
        return dict(status)

    def clear_canonical_overlay(
        self,
        reason: str = "CANONICAL_W3_SOURCE_UNAVAILABLE",
    ) -> dict[str, Any]:
        status = self._canonical.suppress(reason)
        if self._status.get("running") is True:
            self._status["canonicalOverlay"] = status
        return status

    def poll_projection_recovery(
        self,
        client: CdpClient,
        choice: TargetChoice,
        authority_key: str,
        ...
        if self._status.get("running") is True:
            ...
            self._status["projectionProfilesLive"] = (
                self._projection.profiles() is not None
            )
            self._sync_canonical_status()
    ...
    def revoke(self, client: CdpClient | None = None) -> None:
        try:
            self._canonical.revoke("ALPHA_RUNTIME_AUTHORITY_REVOKED")
        except Exception:
            pass
        ...
        if client and self._worker_target_id:
            try:
                self._evaluate(
                    client,
                    self._worker_target_id,
                    "(()=>{try{return self.__WOF_ALPHA_REAL_TRANSPORT?.stop?.('authority-revoked')!==false}catch(_){return false}})()",
                )
            except Exception:
                pass
        if client and self._page_target_id:
            try:
                self._evaluate(
                    ...
                    "(()=>{try{window.__WOF_ALPHA_TRANSPORT_V1?.reset?.()}catch(_){}try{window.WOFALPHAHUD?.transportReset?.()}catch(_){}return true})()",
                )
            except Exception:
                pass
        ...
        self._status = {
            "requested": True,
            "running": False,
            "canonicalOverlay": self._canonical.status(),
            "projectionRecovery": self._projection.status(),
            "projectionProfilesLive": self._projection.profiles() is not None,
            "canonicalSpatialAuthority": "P12->P10->P9/P8->P11",
            "legacySpatialFallback": False,
            **SAFETY,
