...
class RenderAuthorityCapture:
    ...
    def ensure_started(self,client:CdpClient,choice:TargetChoice,authority_key:str,runtime_epoch:str)->dict[str,Any]:
        if self._authority_key==authority_key and self._runtime_epoch==runtime_epoch and self._state in {"MEASURING","ANCHOR_STREAMING","MEASUREMENT_COMPLETE"}:return self.status()
        ...
        self.stop_runtime(client);source=self._verified_text(SOURCE);binding={"worldSha256":WORLD_SHA256,"authorityKey":authority_key,"runtimeEpoch":runtime_epoch,"locator":{"heapBase":locator["heapBase"],"swap16":locator["swap16"]},"continuousActorFeed":True,"readOnly":True,"ramWrites":0,"inputInjection":False}
        try:
        ...
        self._validate_remote(remote,authority_key=authority_key,runtime_epoch=runtime_epoch);self._authority_key=authority_key;self._runtime_epoch=runtime_epoch;self._worker_id=worker_id;self._page_id=page_id;self._state=str(remote.get("state") or "MEASURING");self._error=None;self._result=None;return self.status()
