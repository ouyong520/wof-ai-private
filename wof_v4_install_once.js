...
function __WOF_START_V4(DB){
  ...
  function resetPlayerRuntime(name){
    ...
    const A=AUD?.[name];if(A){A.pending=null;A.prevHp=null;A.lastWarnAt=-1e9;A.lastRawWarnAt=-1e9;A.lastShadowWarnAt=-1e9;A.lastShadowRawAt=-1e9;A.lastGuardWarnAt=-1e9;A.lastGuardRawAt=-1e9;A.lastTailWarnAt=-1e9;A.lastBridgeWarnAt=-1e9;A.lastWatchEvidence=null;A.recent={};A.dead=false;A.absent=false;A.suppressUntil=-1e9;A.lastDamageAt=-1e9;}
  ...
  function addActiveGuard(o,s,now,out){
    ...
    const rx=Math.max(CFG.activeGuardMinRx,Math.min(CFG.activeGuardMaxRx,baseRx*1.22));
    const ry=Math.max(CFG.activeGuardMinRy,Math.min(CFG.activeGuardMaxRy,baseRy*1.22));
    const rz=Math.max(CFG.activeGuardMinRz,Math.min(CFG.activeGuardMaxRz,baseRz*1.22));
...
function auditStep(name,ps,st,raw,now){
  ...
  if(A.dead&&hp>0){
    ...
    A.lastWarnAt=A.lastRawWarnAt=A.lastShadowWarnAt=A.lastShadowRawAt=A.lastGuardWarnAt=A.lastGuardRawAt=-1e9;A.lastWatchEvidence=null;
  ...
  if(dropped){
    if(e&&now>=e.due-CFG.auditHitEarlyMs&&now<=e.deadline+120){
      if(e.attackEndedEarly)auditResolve(name,'ambiguous',e,'预测已撤销后发生 HP '+hpBefore+'→'+hp);
      else if(e.source==='active'||e.familySeen)auditResolve(name,'hit',e,'HP '+hpBefore+'→'+hp);
      ...
      qlog('🟧',name,'原始危险已覆盖/稳定器未确认','HP '+hpBefore+'→'+hp,'case#'+mc.id);
    }else{
...
function summarySnapshot(){
  ...
  const total={tested:0,hit:0,changed:0,enemyChanged:0,ambiguousDamage:0,revoked:0,falsePositive:0,weakFalsePositive:0,watchCovered:0,shadowCovered:0,guardCovered:0,tailCovered:0,bridgeCovered:0,watchPromoted:0,unstableCovered:0,safeMiss:0,nonEnemyDamage:0,hpBaselineReset:0,protectedIgnored:0,deathEvents:0,respawnEvents:0,absentEvents:0,returnEvents:0};
...
function summaryText(){return JSON.stringify(summarySnapshot(),null,2);}
  ...
  self.WOFV4={
    version:'offline-dynamic-spectator-calibrated-v4.11.1',config:CFG,last:null,
  ...
  qlog('✅ WOF V4.11.1 多房间审计去污染/警告负担拆分版启动');
