(async()=>{
'use strict';
const COPY_ID='WOF-034';
const PROJECT='WOF-AI-PRIVATE';
const VERSION='wof-future-danger-adaptive-terminal-miner-v34';
const MARKER='=== WOF FUTURE DANGER ADAPTIVE TERMINAL MINER V34 JSON ===';
...
function mineActive(p,s,t){
  ...
  if(p&&p.attack===0){const lead=t-(history.get(s.slot)?.at(-2)?.t??t-INTERVAL);rec.lastZero={leadMs:r1(Math.max(0,lead)),signature:fp(p),target:p.target,side:p.side,absDx:p.absDx,absDy:p.absDy};const k=`T${s.type}|A${s.attack}|${fp(p)}`;addAgg(terminalAgg,k,{type:s.type,activeAttack:s.attack,signature:fp(p)},Math.max(0,lead),p.target7E===s.target7E,p.side!=null&&s.side!=null?p.side===s.side:null);}
  for(const lag of LAGS){const x=nearestPast(s.slot,t-lag);if(!x)continue;const ps=x.s,sig=fp(ps);rec.lags[String(lag)]={actualLagMs:r1(t-x.t),signature:sig,target:ps.target,side:ps.side,absDx:ps.absDx,absDy:ps.absDy};const k=`${lag}|T${s.type}|A${s.attack}|${sig}`;addAgg(fpAgg,k,{lagMs:lag,type:s.type,activeAttack:s.attack,signature:sig},t-x.t,ps.target7E===s.target7E,ps.side!=null&&s.side!=null?ps.side===s.side:null);}
  const th=(transHist.get(s.slot)||[]).filter(x=>x.t<t&&x.attack===0).slice(-8);for(const x of th){rec.transitions.push({ageMs:r1(t-x.t),signature:x.sig,target:x.s.target,side:x.s.side,absDx:x.s.absDx,absDy:x.s.absDy});}
  for(let j=1;j<th.length;j++){const a=th[j-1],b=th[j],k=`T${s.type}|A${s.attack}|${a.sig}>>${b.sig}`;addAgg(transitionAgg,k,{type:s.type,activeAttack:s.attack,from:a.sig,to:b.sig},t-b.t,b.s.target7E===s.target7E,b.s.side!=null&&s.side!=null?b.s.side===s.side:null);}
  activeEdgeEvents.push(rec);if(activeEdgeEvents.length>180)activeEdgeEvents.shift();
}
function arm(r,s,t){const c=cycle.get(s.slot)||0,k=key(s.slot,r.id);if(armed.get(k)===c)return;armed.set(k,c);const w={id:++wid,rule:r.id,status:r.status,slot:s.slot,type:s.type,cycle:c,horizon:r.horizon,tail:r.tail,at:t,entryTarget7E:s.target7E,entryTarget:s.target,entrySide:s.side,entryAbsDx:s.absDx,resolved:false,outcome:null,leadMs:null,activeAttack:null,expectedAttack:null,targetSame:null,sideSame:null,retargets:[],censored:false};watches.push(w);diag.knownSignals++;events.push({kind:'SIGNAL',rel:r1(t),rule:r.id,slot:s.slot,type:s.type,target:s.target,side:s.side});}
...
function resolveKnown(s,t){for(const w of watches){if(w.resolved||w.slot!==s.slot||w.type!==s.type)continue;const lead=t-w.at;if(lead<0||lead>w.tail)continue;const r=RULES.find(x=>x.id===w.rule);w.resolved=true;w.leadMs=r1(lead);w.activeAttack=s.attack;w.expectedAttack=r.expected.includes(s.attack);w.targetSame=w.entryTarget7E===s.target7E;w.sideSame=w.entrySide===s.side;if(lead<=w.horizon){w.outcome='strictHit';diag.knownStrictHits++;}else if(lead<=w.horizon+JITTER_TOL){w.outcome='jitterBandHit';diag.knownJitterBandHits++;}else{w.outcome='realLateHit';diag.knownRealLateHits++;}events.push({kind:w.outcome,rel:r1(t),rule:w.rule,slot:s.slot,leadMs:w.leadMs,attack:s.attack,expectedAttack:w.expectedAttack});}}
...
function publish(t){const entries=[];for(const w of watches){if(w.resolved)continue;const s=prev.get(w.slot);if(!s||s.type!==w.type)continue;entries.push({copyId:COPY_ID,rule:w.rule,status:w.status,slot:w.slot,target:s.target,target7E:s.target7E,remainingMs:Math.max(0,r1(w.horizon-(t-w.at))),side:s.side,enemyX:s.enemyX,enemyY:s.enemyY,targetX:s.targetX,targetY:s.targetY});}self.__WOF_FUTURE_DANGER_MAP_STATE={copyId:COPY_ID,project:PROJECT,version:VERSION,atMs:r1(t),entries};}
await new Promise(resolve=>{const id=setInterval(()=>{
  ...
  for(let i=0;i<SLOTS;i++){
    ...
    if(!s){if(p){for(const w of watches)if(!w.resolved&&w.slot===i){w.resolved=true;w.censored=true;w.outcome='censored';diag.censored++;}}prev.delete(i);history.delete(i);transHist.delete(i);continue;}
    diag.enemySamples++;typeSamples['T'+s.type]=(typeSamples['T'+s.type]||0)+1;
    if(p&&p.type!==s.type){for(const w of watches)if(!w.resolved&&w.slot===i){w.resolved=true;w.censored=true;w.outcome='censored';diag.censored++;}cycle.set(i,(cycle.get(i)||0)+1);history.delete(i);transHist.delete(i);}
    addHist(s,t);
...
function finalizeAgg(map,limit){return[...map.values()].map(q=>({...q,targetSameRate:q.targetTotal?+(q.targetSame/q.targetTotal).toFixed(3):null,sideStableRate:q.sideTotal?+(q.sideSame/q.sideTotal).toFixed(3):null,leadMin:q.leadSamples.length?Math.min(...q.leadSamples):null,leadMax:q.leadSamples.length?Math.max(...q.leadSamples):null})).sort((a,b)=>b.count-a.count||(a.leadMin??1e9)-(b.leadMin??1e9)).slice(0,limit);}
const knownRuleStats={};for(const r of RULES){const ws=watches.filter(w=>w.rule===r.id),ev=ws.filter(w=>!w.censored),strict=ev.filter(w=>w.outcome==='strictHit').length,jit=ev.filter(w=>w.outcome==='jitterBandHit').length,late=ev.filter(w=>w.outcome==='realLateHit').length,miss=ev.filter(w=>w.outcome==='hardMiss').length,hits=ev.filter(w=>w.leadMs!=null);knownRuleStats[r.id]={status:r.status,signals:ws.length,evaluable:ev.length,strictHit:strict,jitterBandHit:jit,realLateHit:late,hardMiss:miss,censored:ws.filter(w=>w.censored).length,strictRate:ev.length?+(strict/ev.length).toFixed(3):null,expectedAttackRate:hits.length?+(hits.filter(w=>w.expectedAttack).length/hits.length).toFixed(3):null,targetSameRate:hits.length?+(hits.filter(w=>w.targetSame).length/hits.length).toFixed(3):null,sideStableRate:hits.length?+(hits.filter(w=>w.sideSame).length/hits.length).toFixed(3):null,leads:hits.map(w=>w.leadMs).sort((a,b)=>a-b)};}
const out={copyId:COPY_ID,project:PROJECT,version:VERSION,expectedMarker:MARKER,readOnly:true,ramWrites:0,durationRequestedMs:DURATION,actualDurationMs:r1(performance.now()-start),intervalMs:INTERVAL,jitterToleranceMs:JITTER_TOL,model:{purpose:'Coverage-adaptive pre-ACTIVE miner after WOF-033 had291 ACTIVE edges but zero matches for every fixed candidate in that room. Preserve opportunistic validation of T16/T33/T34 while mining exact terminal zero-state, transitions, and20/50/100/150/250/500ms fingerprints for whatever types actually appear.',targetPolicy:'live enemy+0x7E authoritative',activeConvention:'enemy+0x70 U16 0->nonzero; not exact damage/hitbox onset',miningPolicy:'Discovery only. Terminal/fingerprint/transition rankings are correlations until prospectively validated.'},diagnostics:{...diag,typeSamples,attackTypeCounts,rawMatchSamples,transitionEntries},knownRuleStats,terminalTop:finalizeAgg(terminalAgg,100),transitionTop:finalizeAgg(transitionAgg,120),fingerprintTop:finalizeAgg(fpAgg,160),activeEdgeEvents};
console.log(MARKER);console.log(JSON.stringify(out,null,2));return out;
})().catch(e=>{console.error('[WOF-034] ERROR',e);throw e;});
