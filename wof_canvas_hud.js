(()=>{
'use strict';
const CHANNEL='wof-ai-hud-v1',STALE_MS=1500,HOLD_MS={1:1100,2:1300,3:1800},RELEASE_GRACE_MS=420;
...
// Only dispose a reload-safe v4 instance. Never call legacy v3 destroy(), because it
// tries to restore/delete drawArrays and can disturb the current room temporarily.
if(window.WOFHUD?.version?.startsWith?.('canvas-hud-v4')){
...
function drawHud(mode,first,count){
  if(disposed||count!==6)return;
  const now=Date.now(),h=activeHold(now);if(!h)return;
  paint(h,now);
  const W=gl.drawingBufferWidth||canvas.width,H=gl.drawingBufferHeight||canvas.height;if(!(W>0&&H>0))return;
  const w=Math.min(260,W-8),hh=Math.min(86,H-8),x=(W-w)/2,y=Math.max(4,H-hh-8),l=x/W*2-1,r=(x+w)/W*2-1,t=1-y/H*2,b=1-(y+hh)/H*2;
...
function key(e){if(e.code==='F6'){e.preventDefault();cycle();}else if(e.code==='F8'){e.preventDefault();visible=!visible;}else if(e.code==='F9'){e.preventDefault();detail=!detail;lastKey='';}}
...
window.WOFHUD={version:'canvas-hud-v4-reload-safe',p1(){return setFocus('P1')},p2(){return setFocus('P2')},p3(){return setFocus('P3')},focus:setFocus,cycle,show(){visible=true;return true},hide(){visible=false;return true},toggle(){visible=!visible;return visible},detail(v=!detail){detail=!!v;lastKey='';return detail},status(){return{focus,visible,detail,workerConnected:!!lastRx&&Date.now()-lastRx<=STALE_MS,ageMs:lastRx?Date.now()-lastRx:null,drawHooked:gl.drawArrays===bridge.wrapper,bridgeInstalls:bridge.installs,drawCount,lastDrawAt,hold:HOLD[focus],note:'SAFE hidden; player-follow waits for real camera scroll RAM'}},dispose,destroy(){dispose();delete window.WOFHUD;console.log('⛔ WOF HUD disposed; persistent WebGL bridge kept for safe reload');}};
console.log('✅ WOF direct WebGL HUD v4 started | reload-safe persistent draw bridge | SAFE hidden');
console.log('♻️以后可直接重复运行加载命令；不需要先 destroy()');
})();