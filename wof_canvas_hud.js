...
if(!bridge||bridge.gl!==gl||typeof bridge.wrapper!=='function'){
  ...
  bridge.wrapper=function(mode,first,count){
    ...
    if(this===gl&&cb&&!bridge.inCallback){
      ...
      try{cb(mode,first,count);}catch(e){bridge.lastError=String(e?.stack||e);}
...
if(window.WOFHUD?.version?.startsWith?.('canvas-hud-v4')||window.WOFHUD?.version?.startsWith?.('canvas-hud-v5')||window.WOFHUD?.version?.startsWith?.('canvas-hud-v6')){
...
hud.width=300;hud.height=96;const c=hud.getContext('2d');

const HOLD={P1:null,P2:null,P3:null};
const loadedAt=Date.now(),startupUntil=loadedAt+STARTUP_MS;
let focus='P1',visible=true,detail=false,lastMsg=null,lastRx=0,lastKey='',disposed=false,drawCount=0,lastDrawAt=0,callbackCount=0;
...
function paintDanger(h,now){const main=mainText(h),t=heldTime(h,now),threat=sideText(h.p),time=t!=null?(t<200?'立即':'约 '+(t/1000).toFixed(2)+' 秒'):(h.level>=3?'立即':'保持注意'),dbg=detail?[focus,h.p?.source,h.p?.family,h.p?.type!=null?'T'+h.p.type:null,h.p?.slot!=null?'slot '+h.p.slot:null].filter(Boolean).join(' · '):'',key=['danger',main,threat,time,dbg,Math.round((t||0)/50)].join('|');if(key===lastKey)return;lastKey=key;c.clearRect(0,0,hud.width,hud.height);c.fillStyle='rgba(0,0,0,.90)';c.fillRect(1,1,298,94);c.strokeStyle='rgba(255,255,255,.99)';c.lineWidth=2;c.strokeRect(2,2,296,92);c.fillStyle='#fff';c.textAlign='center';c.textBaseline='middle';c.font=h.level>=3?'bold 30px sans-serif':'bold 26px sans-serif';c.fillText(main,150,28);c.font='bold 17px sans-serif';c.fillText(threat,150,58);c.font='bold 13px sans-serif';c.fillText(time,150,80);if(dbg){c.textAlign='left';c.font='9px sans-serif';c.fillText(dbg,7,91);}upload();}
function paintStartup(now){const sec=Math.max(1,Math.ceil((startupUntil-now)/1000)),key='startup|'+sec;if(key===lastKey)return;lastKey=key;c.clearRect(0,0,hud.width,hud.height);c.fillStyle='rgba(0,0,0,.90)';c.fillRect(1,1,298,58);c.strokeStyle='rgba(255,255,255,.99)';c.lineWidth=3;c.strokeRect(2,2,296,56);c.fillStyle='#fff';c.textAlign='center';c.textBaseline='middle';c.font='bold 22px sans-serif';c.fillText('HUD 已加载 · '+sec+'s',150,30);upload();}
...
function drawHud(mode,first,count){
  ...
  if(h){
    paintDanger(h,now);
    const w=Math.min(300,W-8),hh=Math.min(96,H-8),x=(W-w)/2,y=Math.max(4,H-hh-8);
    drawTexture(x,y,w,hh);return;
  }
  if(now<startupUntil){
    paintStartup(now);
    const w=Math.min(300,W-8),hh=Math.min(60,H-8),x=(W-w)/2;
    drawTexture(x,8,w,hh);
  }
}
...
window.WOFHUD={version:'canvas-hud-v6-load-confirm-robust',p1(){return setFocus('P1')},p2(){return setFocus('P2')},p3(){return setFocus('P3')},focus:setFocus,cycle,show(){visible=true;lastKey='';return true},hide(){visible=false;lastKey='';return true},toggle(){visible=!visible;lastKey='';return visible},detail(v=!detail){detail=!!v;lastKey='';return detail},status(){return{focus,visible,detail,workerConnected:!!lastRx&&Date.now()-lastRx<=STALE_MS,ageMs:lastRx?Date.now()-lastRx:null,drawHooked:gl.drawArrays===bridge.wrapper,bridgeInstalls:bridge.installs,gameDraws:bridge.gameDraws,callbackCount,drawCount,lastDrawAt,lastError:bridge.lastError||null,loadConfirmSecondsLeft:Math.max(0,Math.ceil((startupUntil-Date.now())/1000)),hold:HOLD[focus],note:'startup draws after every game drawArrays; SAFE hidden after confirmation'}},dispose,destroy(){dispose();delete window.WOFHUD;console.log('⛔ WOF HUD disposed; persistent WebGL bridge kept for safe reload');}};

// Try once immediately as well. Normal game draws will keep it visible afterward.
try{paintStartup(Date.now());const W=gl.drawingBufferWidth||canvas.width;drawTexture(Math.max(4,(W-Math.min(300,W-8))/2),8,Math.min(300,W-8),60);}catch(_){}
console.log('✅ WOF direct WebGL HUD v6 started | robust 60s in-game confirmation | SAFE hidden');
console.log('♻️ 直接重复运行加载命令即可；不需要先 destroy()');
})();